                                   ...
                                   char *srcaddr, struct addrinfo **targetinfo)
{
  ...
  *targetinfo = gotten;
  ...
  if (sock < 0) {
    fprintf(stderr, "nDAG: Failed to create multicast socket for %s:%s -- %s\n",
            groupaddr, portstr, strerror(errno));
    goto sockcreateover;
  }

  if (setsockopt(
        sock, gotten->ai_family == PF_INET6 ? IPPROTO_IPV6 : IPPROTO_IP,
        gotten->ai_family == PF_INET6 ? IPV6_MULTICAST_HOPS : IP_MULTICAST_TTL,
        (char *)&ttl, sizeof(ttl)) != 0) {
    fprintf(stderr, "nDAG: Failed to configure multicast TTL for %s:%s -- %s\n",
            groupaddr, portstr, strerror(errno));
    close(sock);
    sock = -1;
    goto sockcreateover;
                     ...
                     int compress)
{
  ...
  params->mmsgbufs =
    (struct mmsghdr *)calloc(sizeof(struct mmsghdr), NDAG_BATCH_SIZE);

  for (i = 0; i < NDAG_BATCH_SIZE; i++) {
    params->headerspace[i] = (char *)calloc(1, NDAG_MAX_DGRAM_SIZE);
    params->mmsgbufs[i].msg_hdr.msg_iov =
      (struct iovec *)malloc(sizeof(struct iovec) * 2);
...
static inline char *populate_common_header(char *bufstart, uint16_t monitorid,
                                           uint8_t pkttype)
{
                                ...
                                uint16_t reccount, int index)
{
  ...
  encap = (ndag_encap_t *)(populate_common_header(
    params->headerspace[index], params->monitorid, NDAG_PKT_ENCAPERF));
  ...
  if (params->iovec_count[index] < num_iov + 1) {
    params->mmsgbufs[index].msg_hdr.msg_iov =
      realloc(params->mmsgbufs[index].msg_hdr.msg_iov,
              (num_iov + 1) * sizeof(struct iovec));
  ...
  if (num_iov == 1 && iovecs[0].iov_len > params->maxdgramsize -
    ...
    erfptr->rlen = htons(params->maxdgramsize - sizeof(ndag_common_t) -
                         sizeof(ndag_encap_t));
    iovecs[0].iov_len =
      params->maxdgramsize - sizeof(ndag_common_t) - sizeof(ndag_encap_t);
  }

  for (i = 0; i < num_iov; i++) {
    params->mmsgbufs[index].msg_hdr.msg_iov[i + 1].iov_base =
      iovecs[i].iov_base;
...
#else
  if (syscall(__NR_sendmmsg, params->sock, params->mmsgbufs, msgcount, 0) !=
      msgcount) {
    fprintf(stderr, "Failed to send nDAG encap. records: %s\n",
            strerror(errno));
    reccount = 0;
...
int ndag_send_keepalive(ndag_encap_params_t *params)
{
  ...
  if (sendto(params->sock, alive, sizeof(ndag_common_t), 0,
             ...
             params->target->ai_addrlen) != sizeof(ndag_common_t)) {
...
static uint32_t construct_beacon(char **buffer, ndag_beacon_params_t *nparams)
{
  ...
  uint32_t beacsize =
    sizeof(ndag_common_t) + (sizeof(uint16_t) * (nparams->numstreams + 1));
...
void *ndag_start_beacon(void *params)
{
  ...
  beacsock = ndag_create_multicaster_socket(
    nparams->beaconport, nparams->groupaddr, nparams->srcaddr, &targetinfo);
