...
static int parse_torrents(telescope_global_t *glob, yaml_document_t *doc,
                          yaml_node_t *torrentlist)
{
  ...
  for (item = torrentlist->data.sequence.items.start;
       item != torrentlist->data.sequence.items.top; ++item) {
    yaml_node_t *node;
    ...
    if (needsdefaults) {
      ...
      if (current->name == NULL) {
        fprintf(stderr, "Please specify a name for each multicast sink.\n");
        goto torrentparseerror;
      }
    }
...
static int parse_yaml(telescope_global_t *glob, char *configfile,
