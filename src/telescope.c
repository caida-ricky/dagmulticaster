...
static void append(iov_data_t *iov, uint16_t curiov, int color, char *bottom,
                   uint16_t len, uint32_t *collected)
{
...
static void end(iov_data_t *iov, uint16_t *curiov)
{
  if (iov->vec[*curiov].iov_len != 0) {
    *curiov = *curiov + 1;
    ...
    if (*curiov == iov->len) {
      iov->vec = (struct iovec *)realloc(iov->vec, sizeof(struct iovec) *
                                                     (iov->len + 10));
      iov->len += 10;
    }
...
void telescope_walk_records(char **bottom, char *top, dagstreamthread_t *dst,
                            uint16_t *savedtosend,
                            uint16_t *records_walked_total,
                            ndag_encap_params_t *state)
{
  ...
  do {
    ...
    (*bottom) = walk_stream_buffer((*bottom), top, &records_walked_loop,
                                   available, dst, filter, records_walked);
...
int main(int argc, char **argv)
{
  ...
  while (!is_halted()) {
    if (darkfilter) {
      errorstate = run_dag_streams(dagfd, firstport, beaconcnt, beaconparams,
                                   &params, darkfilter, create_darkfilter,
