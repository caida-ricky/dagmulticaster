                               ...
                               uint16_t streamnum)
{
  ...
  streamconf =
    dag_component_get_subcomponent(root, kComponentStream, streamnum);

  any = dag_component_get_attribute_uuid(streamconf, kStructAttributeMemNode);
...
int init_dag_stream(dagstreamthread_t *dst)
{
  ...
  if (dag_attach_stream64(dst->params.dagfd, dst->params.streamnum, 0,
                          8 * 1024 * 1024) != 0) {
    if (errno == ENOMEM) {
  ...
  if (dag_set_stream_poll64(dst->params.dagfd, dst->params.streamnum, mindata,
                            &maxwait, &poll) != 0) {
                  ...
                  int streamnum, uint64_t globalstart)
{
  ...
  sock =
    ndag_create_multicaster_socket(params->exportport, params->multicastgroup,
                                   params->sourceaddr, &targetinfo);
  if (sock == -1) {
...
static inline void log_stats(dagstreamthread_t *dst, struct timeval now)
{
  ...
  if (dst->params.statdir) {
    snprintf(buf, sizeof(buf), "%s/ndag.stream-%02d.stats", dst->params.statdir,
             dst->params.streamnum);
    if ((logf = wandio_wcreate(buf, WANDIO_COMPRESS_NONE, 0, O_CREAT)) ==
        NULL) {
    ...
    wandio_printf(logf,
                  ...
                  (int)now.tv_sec, dst->params.statinterval,
                  dst->params.streamnum, dst->stats.walked_buffers,
                  dst->stats.walked_records, dst->stats.walked_bytes,
                  dst->stats.walked_wbytes, dst->stats.filtered_out.tx_records,
                  dst->stats.filtered_out.tx_bytes,
                  dst->stats.filtered_out.tx_wbytes, dst->stats.dropped_records,
    ...
    for (i = 0; i < dst->inuse; ++i) {
      wandio_printf(logf,
                    ...
                    dst->stats.sinks[i].name, dst->stats.sinks[i].tx_datagrams,
                    dst->stats.sinks[i].name, dst->stats.sinks[i].tx_records,
                    dst->stats.sinks[i].name, dst->stats.sinks[i].tx_bytes,
                    dst->stats.sinks[i].name, dst->stats.sinks[i].tx_wbytes);
    }
    ...
    fprintf(stderr,
            ...
            "dropped_records:%" PRIu64 " "
            "truncated_records %" PRIu64 "\n",
            (int)now.tv_sec, dst->params.streamnum, dst->stats.walked_buffers,
            dst->stats.walked_records, dst->stats.walked_bytes,
            dst->stats.walked_wbytes, dst->stats.filtered_out.tx_records,
            dst->stats.filtered_out.tx_bytes, dst->stats.filtered_out.tx_wbytes,
            dst->stats.dropped_records, dst->stats.truncated_records);
    for (i = 0; i < dst->inuse; ++i) {
      fprintf(stderr,
              "%s_tx_datagrams %" PRIu64 "\n"
              "%s_tx_records %" PRIu64 "\n"
                                          ...
                                          ndag_encap_params_t *))
{
  ...
  fprintf(stderr,
          "Halting stream %d after processing %lu records in %d.%d seconds\n",
          dst->params.streamnum, dst->stats.walked_records,
          (int)timetaken.tv_sec, (int)timetaken.tv_usec);
}
...
static int start_dag_thread(dagstreamthread_t *nextslot, uint8_t *cpumap,
                            void *(*processfunc)(void *))
{
  ...
  /* Attach to a stream */
  if (dag_attach_stream64(nextslot->params.dagfd, nextslot->params.streamnum, 0,
                          8 * 1024 * 1024) != 0) {
    if (errno == ENOMEM) {
  ...
  pthread_attr_init(&attrib);
  pthread_attr_setaffinity_np(&attrib, sizeof(cpus), &cpus);
  ret = pthread_create(&nextslot->tid, &attrib, processfunc, (void *)nextslot);
  pthread_attr_destroy(&attrib);

#else
  ret = pthread_create(&nextslot->tid, NULL, processfunc, (void *)nextslot);
#endif
...
static void dst_destroy(dagstreamthread_t *dst, void (*destroyfunc)(void *))
{
  int i;
                    ...
                    void *(*processfunc)(void *), void (*destroyfunc)(void *))
{
  ...
  dagthreads =
    (dagstreamthread_t *)(malloc(sizeof(dagstreamthread_t) * maxstreams));
  if (dagthreads == NULL) {
