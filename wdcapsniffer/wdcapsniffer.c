...
static char *walk_stream_buffer(char *bottom, char *top, uint16_t *reccount,
                                uint16_t *curiov, dagstreamthread_t *dst,
                                wdcapdata_t *wdcap)
{
      ...
      if (snapto < len) {
        ...
        *curiov = *curiov + 1;
        ...
        if (*curiov == dst->iov_alloc) {
          dst->iovs = (struct iovec *)realloc(
            dst->iovs, sizeof(struct iovec) * (dst->iov_alloc + 10));
          dst->iov_alloc += 10;
        }
                            ...
                            uint16_t *savedtosend, ndag_encap_params_t *state)
{
  ...
  do {
    records_walked = 0;
    (*bottom) = walk_stream_buffer((*bottom), top, &records_walked, &available,
                                   dst, wdcap);
...
int main(int argc, char **argv)
{
  ...
  while (1) {
    ...
    static struct option long_options[] = {
      {"device", 1, 0, 'd'},    {"help", 0, 0, 'h'},
      {"monitorid", 1, 0, 'm'}, {"beaconport", 1, 0, 'p'},
      {"groupaddr", 1, 0, 'a'}, {"sourceaddr", 1, 0, 's'},
      {"mtu", 1, 0, 'M'},       {"wdcapconfig", 1, 0, 'w'},
      {NULL, 0, 0, 0}};

    c = getopt_long(argc, argv, "w:a:s:d:hm:M:p:", long_options, &option_index);
    if (c == -1)
      break;
  ...
  if (params.monitorid == 0) {
    fprintf(stderr, "0 is not a valid monitor ID -- choose another number.\n");
    goto finalcleanup;
  }
  ...
  params.globalstart =
    bswap_host_to_be64((starttime.tv_sec - 1509494400) * 1000) +
  ...
  while (!is_halted()) {
    errorstate =
      run_dag_streams(dagfd, firstport, &beaconparams, &params, wdcapconf,
                      init_wdcap_data, per_dagstream, destroy_wdcap_data);
