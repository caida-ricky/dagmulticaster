...
int init_dag_stream(dagstreamthread_t *dst, ndag_encap_params_t *state)
{
  ...
  sock = ndag_create_multicaster_socket(dst->params.exportport,
                                        ...
                                        dst->params.sourceaddr, &targetinfo);
  if (sock == -1) {
...
static inline void log_stats(dagstreamthread_t *dst, struct timeval now)
{
  ...
  if (dst->params.statdir) {
    ...
    snprintf(buf, sizeof(buf),
             ...
             (int)now.tv_sec, dst->params.statinterval, dst->params.streamnum,
             dst->stats.walked_buffers, dst->stats.walked_records,
             dst->stats.walked_bytes, dst->stats.walked_wbytes,
             dst->stats.tx_datagrams, dst->stats.tx_records,
             dst->stats.tx_bytes, dst->stats.tx_wbytes,
    ...
    fprintf(stderr,
            ...
            dst->stats.walked_wbytes, dst->stats.tx_datagrams,
            dst->stats.tx_records, dst->stats.tx_bytes, dst->stats.tx_wbytes,
            dst->stats.dropped_records, dst->stats.truncated_records);
  }
}

void dag_stream_loop(dagstreamthread_t *dst, ndag_encap_params_t *state,
                     uint16_t (*walk_records)(char **, char *,
                                              dagstreamthread_t *, uint16_t *,
                                              ndag_encap_params_t *))
{
  void *bottom, *top;
  ...
  while (!halted && !paused) {
    ...
    reccnt =
      walk_records((char **)(&bottom), (char *)top, dst, &savedtosend, state);
